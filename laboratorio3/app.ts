...
app.get('/get_data', (req: any, res: any) => {
...
app.get('/search_rut', jsonParser, (req: any, res: any) => {
    ...
    fs.readFile('./data.json', 'utf8', (err: any, data: any) => {
        ...
        try {
            const jsonData = JSON.parse(data);
            const newElement = jsonData.find((x: any) => x.rut === rut);
...
app.post('/add_data', jsonParser, (req: any, res: any) => {
    ...
    fs.readFile('./data.json', 'utf8', (err: any, data: any) => {
        ...
        try {
            const jsonData = JSON.parse(data);
            const exist = jsonData.some((x: any) => x.rut === new_element.rut)
...
app.put('/update_data/:rut', jsonParser, (req: any, res: any) => {
    const rut = req.body.rut;
    const new_element = req.body;
...
app.delete('/delete_data/:rut', jsonParser, (req: any, res: any) => {
    const rut = req.body.rut;
    ...
    fs.readFile('./data.json', 'utf8', (err: any, data: any) => {
        ...
        try {
            ...
            const exist = jsonData.some((x: any) => x.rut === rut);

            if(!exist) {
                return res.status(200).send(`No existe el rut ${rut}`);
            }

            jsonData = jsonData.filter((x: any) => x.rut !== rut);
